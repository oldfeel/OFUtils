...
func generateSingleFileBackup(options Options, db string) {
	...
	printMessage("mysqldump is being executed with parameters : "+options.MySQLDumpPath+" "+strings.Join(args, " "), options.Verbosity, Info)

	cmd := exec.Command(options.MySQLDumpPath, args...)
	cmdOut, e := cmd.StdoutPipe()
	printE(e, 1)
	cmdErr, e := cmd.StderrPipe()
	printE(e, 2)

	cmd.Start()

	output, e := ioutil.ReadAll(cmdOut)
	printE(e, 3)
	err, e := ioutil.ReadAll(cmdErr)
	printE(e, 4)
	cmd.Wait()
...
func printE(e error, p int) {
	if e != nil {
		printMessage(ToString(p)+" "+e.Error(), 2, Error)
	}
}
